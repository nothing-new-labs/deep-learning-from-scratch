import sys, os
sys.path.append(os.pardir) # 为了导入父目录文件的设置
import numpy as np
...
from common.batch_normalization import BatchNormalization
...
class MultiLayerNetExtend:
    ...
    具有Weight Decay、Dropout、Batch Normalization功能

    Parameters
    ----------
    input_size : 输入大小(MNIST情况下为784)
    hidden_size_list : 隐藏层神经元数量的列表(例如：[100, 100, 100])
    output_size : 输出大小(MNIST情况下为10)
    activation : 'relu' or 'sigmoid'
    weight_init_std : 指定权重的标准差(例如:0.01)
        指定'relu'或'he'时设置「He初始值」
        指定'sigmoid'或'xavier'时设置「Xavier初始值」
    ...
    def __init_weight(self, weight_init_std):
        ...
        for idx in range(1, len(all_size_list)):
            ...
            if str(weight_init_std).lower() in ('relu', 'he'):
                scale = np.sqrt(2.0 / all_size_list[idx - 1])  # 使用ReLU时推荐的初始值
